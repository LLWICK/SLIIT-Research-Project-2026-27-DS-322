...
def render(data: dict[str, Any]) -> None:
    ...
    with st.container(border=True):
        section_title("Why LSTM can look better on MAE", "Do not ship an uncalibrated winner")
        picp = lstm["picp"] if lstm else "—"
        st.caption(
            f"The optional MLP can post a lower point MAE, but its PICP is only {picp}% "
            "with a much tighter band. LightGBM + CQR is the primary model because coverage stays near 90% "
            "and the interval stays usable."
        )
